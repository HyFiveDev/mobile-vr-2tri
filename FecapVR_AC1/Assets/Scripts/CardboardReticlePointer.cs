...
public class CardboardReticlePointer : MonoBehaviour
{
    ...
    ///<summary>
    ///Modificação para o uso do InputSystem
    /// </summary>
    private InputAction fireOne, fireTwo;

    StarterAssetsMaps starterAssetsMaps;
    private void Awake()
    {
        starterAssetsMaps = new StarterAssetsMaps();
        fireOne = starterAssetsMaps.Player.FireOne;
        fireTwo = starterAssetsMaps.Player.FireTwo;
    }
    ...
    private void Update()
    {
        ...
        // at.
        //Aqui eu coloquei o ReticleInteractionLayerMask, estava faltando no código original
        RaycastHit hit;
        ...
        if (fireOne.WasPerformedThisFrame())
            _gazedAtObject?.SendMessage("OnPointerClick", null, SendMessageOptions.DontRequireReceiver);

        if (fireOne.WasReleasedThisFrame())
            _gazedAtObject?.SendMessage("OnPointerUp", null, SendMessageOptions.DontRequireReceiver);

        if (fireOne.WasPressedThisFrame())
            _gazedAtObject?.SendMessage("OnPointerDown", null, SendMessageOptions.DontRequireReceiver);

        if (fireTwo.WasPressedThisFrame())
            _gazedAtObject?.SendMessage("OnFireTwoClick", null, SendMessageOptions.DontRequireReceiver);
